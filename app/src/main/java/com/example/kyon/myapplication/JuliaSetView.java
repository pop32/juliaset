...
public class JuliaSetView extends View implements View.OnClickListener, View.OnTouchListener {
    ...
    private DrawOrder drawOrder = null;
    private TouchEvent eTouchEvent = TouchEvent.nothing;
    ...
    public JuliaSetView(Context context) {
        ...
        canvasBuf.drawText("please tap screen", juliaSetScreenSize/2,juliaSetScreenSize/2,paint);

        //new Thread(new DrawOrder(true)).start();
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        Log.d(JuliaSetView.class.getName(), "onDraw Start");
        canvas.drawBitmap(bmpBuf,0,0,null);
        Log.d(JuliaSetView.class.getName(), "onDraw End");
    }

    @Override
    public void onClick(View v) {
        Log.d(JuliaSetView.class.getName(), "onClick start:");
        synchronized (isCalc) {
            if (isCalc) {
                return;
            }
...
//                    t3.join();
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        if (event.getActionMasked() == MotionEvent.ACTION_DOWN) {
            ...
            if (touchY < juliaSetScreenSize) {
                eTouchEvent = TouchEvent.doubleJuliaSet;
            } else {
                eTouchEvent = TouchEvent.newJuliaSet;
            }
    ...
    private class DrawOrder implements Runnable {
        ...
        public void run() {
            synchronized (isCalc) {
                ...
                isCalc = true;
                Log.d(JuliaSetView.class.getName(), "run calc start");
